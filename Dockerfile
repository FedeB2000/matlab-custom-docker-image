# Usa questo Dockerfile per costruire una container image MATLAB
# con i prodotti e i support package che vuoi installare.

# Argomento di build che specifica la release MATLAB da usare.
# Puoi passarlo con:  --build-arg MATLAB_RELEASE=R20XXx
# L'immagine di base è quella "matlab-deps" su Docker Hub.
ARG MATLAB_RELEASE=R2025b 

# Fase principale: usa l'immagine ufficiale MathWorks con le dipendenze di MATLAB,
# basata su Ubuntu, per la release indicata.
FROM mathworks/matlab-deps:${MATLAB_RELEASE}

# Ridefiniamo l'ARG nella build stage corrente (necessario se lo vuoi riusare
# dopo l'istruzione FROM).
ARG MATLAB_RELEASE=R2025b

# Cartella di installazione di MATLAB all'interno del container.
ARG DESTINATION_FOLDER="/usr/local/MATLAB/${MATLAB_RELEASE}"

# Livello di aggiornamento da installare:
# 0 = release base, 1 = primo update (Update 1), ecc.
ARG UPDATELEVEL=1

# Elenco dei prodotti da installare.
ARG PRODUCTS
...
## Qui hai l’elenco dei prodotti MATLAB/Simulink.
## Ogni riga è commentata (#). Per installare un prodotto,
## basta togliere il # davanti a "product.Nome_Prodotto".
## Esempio:
##   product.MATLAB
##   product.Simulink
## Verranno poi raccolti automaticamente più avanti.
...
## Stesso discorso dei prodotti: ogni riga è un support package o una libreria.
## Per includerlo nell’installazione, basta togliere il # davanti a "product.<nome>".
...
# Qui si vuole estrarre tutte le righe non commentate che iniziano con "product."
# dal Dockerfile stesso, trasformando "product.X" in una lista di nomi "X" separati da spazio.
# ATTENZIONE: così com’è, questa riga ha due problemi:
#   1) Nel container non è detto che esista un file chiamato "Dockerfile".
#   2) La variabile $PRODUCTS vive solo in questa singola RUN e NON sarà
#      disponibile nelle RUN successive (dove tentando di usarla).
RUN PRODUCTS=$(grep -E "^[^#]*product\." Dockerfile | sed 's/^product\.//' | tr '\n' ' ') \
    && echo "Prodotti selezionati: ${PRODUCTS}"

# Installa le dipendenze necessarie per usare "mpm" (MathWorks Product Manager):
# wget per scaricare il binario, ca-certificates per HTTPS.
...
# Crea un utente non-root chiamato "matlab" e gli dà permessi sudo senza password.
# Buona pratica di sicurezza: evita di eseguire MATLAB come root.
RUN adduser --shell /bin/bash --disabled-password --gecos "" matlab \
    && echo "matlab ALL=(ALL) NOPASSWD: ALL" > /etc/sudoers.d/matlab \
    && chmod 0440 /etc/sudoers.d/matlab

# Imposta l’utente di default del container e la working directory.
USER matlab
WORKDIR /home/matlab

# Scarica il binario "mpm" (MathWorks Product Manager) per Linux x64
# e lo rende eseguibile.
RUN wget -q https://www.mathworks.com/mpm/glnxa64/mpm \
    && chmod +x mpm 

# Esegue l’installazione di MATLAB con mpm.
# - HOME=${HOME}: forza la home corretta per l'utente matlab.
# - --release=${MATLAB_RELEASE}U${UPDATELEVEL}: release + livello di update (es. R2025bU1).
# - --destination=${DESTINATION_FOLDER}: directory di installazione.
# - --products ${PRODUCTS}: elenco prodotti da installare.
# Se l’installazione fallisce:
#   - stampa il log /tmp/mathworks_root.log
#   - termina con errore (false).
# ATTENZIONE: qui ${PRODUCTS} NON è definita, perché è stata definita solo
# in una RUN precedente e non è un ENV. Così com’è, questo comando fallisce.
RUN sudo HOME=${HOME} ./mpm install \
         --release=${MATLAB_RELEASE}U${UPDATELEVEL} \
...
# - MATLAB_ROOT: posizione dell’installazione MATLAB.
# - PATH: aggiunge la cartella bin di MATLAB al PATH.
ENV MATLAB_ROOT=${DESTINATION_FOLDER} \
    PATH=${DESTINATION_FOLDER}/bin:${PATH}

# Pulizia dei file temporanei per ridurre la dimensione dell'immagine.
# - rimuove /tmp e /var/tmp.
# - rimuove il binario mpm e il log di installazione.
# - crea un link simbolico 'matlab' in /usr/local/bin.
# ATTENZIONE: qui usi ${MATLAB_INSTALL_LOCATION}, che non è mai stato definito.
# Probabilmente volevi usare ${MATLAB_ROOT} oppure ${DESTINATION_FOLDER}.
RUN rm -rf /tmp/* /var/tmp/* \
    && sudo rm -rf mpm /tmp/mathworks_root.log \
    && sudo ln -s ${MATLAB_INSTALL_LOCATION}/bin/matlab /usr/local/bin/matlab

# Imposta l’entrypoint del container in modo che esegua MATLAB.
# Quando fai: docker run <image> ..., parte l’eseguibile "matlab".
ENTRYPOINT ["matlab"]

# Argomenti di default passati a MATLAB.
# Qui è vuoto (stringa vuota), quindi di fatto lanci "matlab" senza opzioni.
CMD [""]
